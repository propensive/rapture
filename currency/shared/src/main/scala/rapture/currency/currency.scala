...
object Currency {
  ...
  object Arithmetic extends Arithmetic_1 {
    ...
    implicit def currencyBaskets[C <: Currency.Key, C2 <: Currency.Key]: Arithmetic[CurrencyBasket[C], CurrencyBasket[C2]] { type Return = CurrencyBasket[C with C2] } =
      new Arithmetic[CurrencyBasket[C], CurrencyBasket[C2]] {
        ...
        def add(left: CurrencyBasket[C], right: CurrencyBasket[C2]): CurrencyBasket[C with C2] =
          CurrencyBasket[C with C2](left.amounts.foldLeft(right.amounts) { case (acc, (k, v)) =>
            acc.updated(k, acc.get(k).getOrElse(0.0) + v)
          })
      }
    
    implicit def currencyBasketAndMoney[C <: Currency.Key, C2 <: Currency.Key]: Arithmetic[CurrencyBasket[C], Money[C2]] { type Return = CurrencyBasket[C with C2] } =
      new Arithmetic[CurrencyBasket[C], Money[C2]] {
        type Return = CurrencyBasket[C with C2]
        def add(left: CurrencyBasket[C], right: Money[C2]): CurrencyBasket[C with C2] =
          CurrencyBasket[C with C2](left.amounts.updated(right.currency, left.amounts.get(right.currency).getOrElse(0.0) + right.amount))
      }
    
    implicit def moneyAndCurrencyBasket[C <: Currency.Key, C2 <: Currency.Key]: Arithmetic[Money[C], CurrencyBasket[C2]] { type Return = CurrencyBasket[C with C2] } =
      new Arithmetic[Money[C], CurrencyBasket[C2]] {
        type Return = CurrencyBasket[C with C2]
        def add(left: Money[C], right: CurrencyBasket[C2]): CurrencyBasket[C with C2] =
          CurrencyBasket[C with C2](right.amounts.updated(left.currency, right.amounts.get(left.currency).getOrElse(0.0) + left.amount))
      }
  }

  trait Arithmetic_1 {
    implicit def differentCurrencies[C <: Currency.Key, C2 <: Currency.Key]: Arithmetic[Money[C], Money[C2]] { type Return = CurrencyBasket[C with C2] } =
...
case class InvalidMoney(currency: Currency) extends Exception(s"this is not a valid money value of currency ${currency.code}")
...
object Money {
  ...
  implicit def moneyParser[C <: Currency.Key](implicit ev: Currency.Evidence[C]): StringParser[Money[C]] { type Throws = InvalidMoney } =
    new StringParser[Money[C]] {
      ...
      def parse(s: String, mode: Mode[_ <: MethodConstraint]): mode.Wrap[Money[C], InvalidMoney] = mode.wrap {
        if(s.substring(s.length - 3) == ev.currency.code) {
          try Money[C](ev.currency, s.dropRight(3).toDouble) catch { case e: Exception => throw InvalidMoney(ev.currency) }
...
case class Money[C <: Currency.Key](currency: Currency { type Key = C }, amount: Double) {
  ...
  def +[C2 <: Currency.Key](m: Money[C2])(implicit arithmetic: Currency.Arithmetic[Money[C], Money[C2]]): arithmetic.Return =
...
case class CurrencyBasket[C <: Currency.Key](amounts: Map[Currency, Double]) {
  ...
  def *[N: Numeric](times: N): CurrencyBasket[C] = CurrencyBasket(amounts.mapValues(_*implicitly[Numeric[N]].toDouble(times)))
  ...
  override def toString = amounts.map { case (currency, value) =>
    implicit val df: DecimalFormat = DecimalPlaces(currency.decimalPlaces)
