...
case class CsvTypeMismatch(typeName: String, col: Int)
    extends CsvGetException {
  override def getMessage =
    s"type mismatch: Could not read value of type $typeName in column $col"
}
...
class CsvRow(val elems: Seq[String]) {
  ...
  def as[T](implicit mode: Mode[`CsvRow#as`],
            extractor: CsvRowExtractor[T]): mode.Wrap[T, CsvGetException] =
...
object CsvRowExtractor {
  implicit def generateExtractor[T]: CsvRowExtractor[T] = macro Macros
    .extractorMacro[T]
}
trait CsvRowExtractor[T] {
  def extract(
      values: Seq[String], mode: Mode[_]): mode.Wrap[T, CsvGetException]
}

case class BadLength(len: Int) extends Exception("Bad length: " + len)

object CsvCellExtractor {
  implicit val stringExtractor: CsvCellExtractor[String] =
    new CsvCellExtractor[String] {
      type ExceptionType = CsvGetException
      def extract(value: String,
                  c: Int,
                  mode: Mode[_]): mode.Wrap[String, CsvGetException] =
        mode.wrap(value)
    }

  implicit val intExtractor: CsvCellExtractor[Int] =
    new CsvCellExtractor[Int] {
      type ExceptionType = CsvGetException
      def extract(value: String,
                  c: Int,
                  mode: Mode[_]): mode.Wrap[Int, CsvGetException] =
        mode.wrap {
          try value.toInt catch {
            case e: Exception => mode.exception(CsvTypeMismatch("integer", c))
          }
        }
    }

  implicit val doubleExtractor: CsvCellExtractor[Double] =
    new CsvCellExtractor[Double] {
      type ExceptionType = CsvGetException
      def extract(value: String,
                  c: Int,
                  mode: Mode[_]): mode.Wrap[Double, CsvGetException] =
        mode.wrap {
          try value.toDouble catch {
            case e: Exception => mode.exception(CsvTypeMismatch("double", c))
          }
        }
    }

  implicit val charExtractor: CsvCellExtractor[Char] =
    new CsvCellExtractor[Char] {
      type ExceptionType = BadLength
      def extract(
          value: String,
          c: Int,
          mode: Mode[_]): mode.Wrap[Char, CsvGetException with BadLength] =
        mode.wrap {
...
trait CsvCellExtractor[T] {
  ...
  def extract(value: String,
              c: Int,
              mode: Mode[_]): mode.Wrap[T, CsvGetException with ExceptionType]
}

case class CsvCell(value: String, col: Int) {
  def as[T](mode: Mode[`CsvCell#as`])(ext: CsvCellExtractor[T]
  ): mode.Wrap[T, CsvGetException with ext.ExceptionType] =
...
case class Csv(rows: Vector[CsvRow]) extends Seq[CsvRow] {
  ...
  override def toString =
    rows.take(4).mkString("\n") + (if (rows.length > 4) "\n..." else "")

  def mapAs[T : CsvRowExtractor](
      implicit mode: Mode[`Csv#mapAs`]): mode.Wrap[Seq[T], CsvGetException] =
    mode.wrap {
      rows.map { row =>
...
object Csv {
  def parse[Res](resource: Res)(
      implicit mode: Mode[`Csv.parse`],
...
trait CsvBackend {
  def parseRow(s: String,
               line: Int,
               mode: Mode[_]): mode.Wrap[Seq[String], CsvParseException]
}
