...
object Sizable {
  class Capability[Res](res: Res) {
    ...
    def size[Data](implicit mode: Mode[`Sizable#size`],
                   sizable: Sizable[Res, Data]): mode.Wrap[Long, Exception] =
      mode wrap sizable.size(res)
  }

  implicit def charSizable[Res, Data : ClassTag](
      implicit reader: Reader[Res, Data]): Sizable[Res, Data] =
    new Sizable[Res, Data] {
      private def accumulator() =
        new Accumulator[Data, Long] with Output[Data] {
          private var count = 0
