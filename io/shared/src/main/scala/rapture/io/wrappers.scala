...
object ClasspathStream {
  def classpathStreamByteReader(implicit cl: ClassLoader) =
    new JavaInputStreamReader[ClasspathUrl](url =>
...
class CharInput(in: java.io.Reader) extends Input[Char] {
  ...
  override def readBlock(
      array: Array[Char], offset: Int = 0, length: Int = -1): Int =
    bin.read(
        array, offset, if (length == -1) (array.length - offset) else length)
...
class ByteInput(in: InputStream) extends Input[Byte] {
  ...
  override def readBlock(
      array: Array[Byte], offset: Int = 0, length: Int = -1): Int =
    bin.read(
        array, offset, if (length == -1) (array.length - offset) else length)
...
class ByteOutput(out: OutputStream, closer: OutputStream => Unit = (_.close()))
    extends Output[Byte] {
...
class JavaOutputStreamWriter[T](val getOutputStream: T => OutputStream,
                                val closer: OutputStream => Unit = (_.close()))
    extends Writer[T, Byte] {
  def output(t: T): Output[Byte] =
    alloc[ByteOutput](alloc[BufferedOutputStream](getOutputStream(t)), closer)
}

class JavaOutputAppender[T](val getOutputStream: T => OutputStream,
                            val closer: OutputStream => Unit = (_.close()))
    extends Appender[T, Byte] {
  def appendOutput(t: T): Output[Byte] =
    alloc[ByteOutput](alloc[BufferedOutputStream](getOutputStream(t)), closer)
}

class JavaInputStreamReader[T](val getInputStream: T => InputStream)
    extends Reader[T, Byte] {
  def input(t: T): Input[Byte] =
    alloc[ByteInput](alloc[BufferedInputStream](getInputStream(t)))
}
