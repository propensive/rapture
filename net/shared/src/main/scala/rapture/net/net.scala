...
object HttpTimeout {
  implicit val defaultHttpTimeout: HttpTimeout {
  ...
  def apply[T : TimeSystem.ByDuration](timeout: T) =
    new HttpTimeout(
        Math
          .min(Int.MaxValue, ?[TimeSystem.ByDuration[T]].fromDuration(timeout))
          .toInt) {
      type Throws = TimeoutException
    }
}
...
object HttpCertificateConfig {
  implicit val defaultHttpCertificateConfig: HttpCertificateConfig {
    type Throws = InvalidCertificateException
  } = new HttpCertificateConfig(true) {
    type Throws = InvalidCertificateException
  }
}
object HttpRedirectConfig {
  implicit val defaultHttpRedirectConfig: HttpRedirectConfig =
    new HttpRedirectConfig(true)
}
class HttpRedirectConfig(val follow: Boolean)

object HttpBasicAuthentication {
  implicit val defaultHttpBasicAuthentication: HttpBasicAuthentication =
    new HttpBasicAuthentication(None)
}
...
object httpOptions {
  object noTimeout {
    implicit val implicitHttpTimeout: HttpTimeout { type Throws = Nothing } =
      new HttpTimeout(-1) {
        type Throws = Nothing
      }
  }
  ...
  object ignoreInvalidCertificates {
    implicit val implicitCertificateConfig: HttpCertificateConfig =
      new HttpCertificateConfig(true)
  }

  object doNotFollowRedirects {
    implicit val implicitFollowRedirects: HttpRedirectConfig =
      new HttpRedirectConfig(false)
  }
}
...
class HttpResponse(
  ...
  def input[Data](
      implicit ib: InputBuilder[InputStream, Data],
...
object PostType {
  ...
  implicit def formPostType: PostType[Map[Symbol, String]] =
    new PostType[Map[Symbol, String]] {
      def contentType = Some(MimeTypes.`application/x-www-form-urlencoded`)
      def sender(content: Map[Symbol, String]) =
        ByteArrayInput(
            (content map {
              case (k, v) =>
                java.net.URLEncoder.encode(k.name, "UTF-8") + "=" +
                java.net.URLEncoder.encode(v, "UTF-8")
            } mkString "&").getBytes("UTF-8"))
    }

  implicit def stringPostType[S : StringSerializer](
      implicit enc: Encoding): PostType[S] = new PostType[S] {
    def contentType = Some(MimeTypes.`text/plain`)
    def sender(content: S) =
      implicitly[StringSerializer[S]].serialize(content).input[Byte]
  }
}
trait PostType[-C] {
...
object NetUrl {
  ...
  implicit val base64: ByteCodec[Base64Padded] =
    new Base64Codec[Base64Padded](endPadding = true)
}
...
trait NetUrl {
  ...
  NetUrl.sslContext.init(
      null, trustAllCertificates, new java.security.SecureRandom())
...
object HttpUrl {
  implicit val parser: StringParser[HttpUrl] = new StringParser[HttpUrl] {
    ...
    def parse(s: String,
              mode: Mode[_ <: MethodConstraint]): mode.Wrap[HttpUrl, Throws] =
      mode.wrap(Http.parse(s))
  }

  implicit val serializer: StringSerializer[HttpUrl] =
    new StringSerializer[HttpUrl] {
  ...
  implicit def uriCapable: UriCapable[HttpUrl] = new UriCapable[HttpUrl] {
    def uri(cp: HttpUrl) = {
      val portString =
        if (cp.ssl && cp.port == 443 || !cp.ssl && cp.port == 80) ""
        else s":${cp.port}"
      Uri(if (cp.ssl) "https" else "http",
          s"//${cp.hostname}${portString}/${cp.elements.mkString("/")}")
    }
  }

  implicit def urlSlashRootedPath[RP <: RootedPath]: Dereferenceable[
      HttpUrl, RP, HttpUrl] =
    new Dereferenceable[HttpUrl, RP, HttpUrl] {
      def dereference(p1: HttpUrl, p2: RP) = {
        val start =
          if (p1.elements.lastOption == Some("")) p1.elements.init
          else p1.elements
        HttpUrl(p1.root, start ++ p2.elements)
      }
    }

  implicit def urlSlashRelativePath[RP <: RelativePath]: Dereferenceable[
      HttpUrl, RP, HttpUrl] =
    new Dereferenceable[HttpUrl, RP, HttpUrl] {
      def dereference(p1: HttpUrl, p2: RP) =
  ...
  implicit def urlParentable: Parentable[HttpUrl, HttpUrl] =
    new Parentable[HttpUrl, HttpUrl] {
      def parent(httpUrl: HttpUrl): HttpUrl =
        HttpUrl(httpUrl.root, httpUrl.elements.dropRight(1))
    }
}
...
case class HttpUrl(root: HttpDomain, elements: Vector[String]) extends NetUrl {
  ...
  def query[Q : Query](q: Q): HttpQuery =
    HttpQuery(this, ?[Query[Q]].queryString(q))
}
...
object Query {
  implicit def mapQuery[K : StringSerializer, V : StringSerializer]: Query[Map[
          K, V]] = new Query[Map[K, V]] {
    def queryString(m: Map[K, V]): String =
      m.map {
        case (k, v) =>
          val key = java.net.URLEncoder.encode(
              ?[StringSerializer[K]].serialize(k), "UTF-8")
          val value = java.net.URLEncoder.encode(
              ?[StringSerializer[V]].serialize(v), "UTF-8")
          s"$key=$value"
      }.mkString("&")
  }
}
...
object HttpQuery {
  implicit val serializer: StringSerializer[HttpQuery] =
    new StringSerializer[HttpQuery] {
  ...
  implicit def uriCapable: UriCapable[HttpQuery] = new UriCapable[HttpQuery] {
    def uri(hq: HttpQuery) = {
      ...
      Uri(httpUrlUri.scheme,
          s"${httpUrlUri.schemeSpecificPart}?${hq.queryString}")
    }
  }
}
