...
object Ipv6 {
  def parse(s: String)(
      implicit mode: Mode[`Ipv6.parse`]): mode.Wrap[Ipv6, Exception] =
    mode.wrap {
      val groups: Array[String] = s.split("::").map(_.split(":")) match {
        ...
        case Array(xs, ys) =>
          xs ++ Array.fill(8 - xs.length - ys.length)("") ++ ys
      }
      val gs =
        groups map (decode[Hex](_).bytes) map {
          case Array() => 0
          case Array(le) => le
...
object Ipv4 {
  ...
  def fromLong(lng: Long) =
    Ipv4((lng >> 24 & 255L).toInt,
         (lng >> 16 & 255L).toInt,
         (lng >> 8 & 255L).toInt,
         (lng & 255L).toInt)

  lazy val privateSubnets = List(Ipv4(10, 0, 0, 0) / 8,
                                 Ipv4(192, 168, 0, 0) / 16,
                                 Ipv4(172, 16, 0, 0) / 12,
                                 Ipv4(127, 0, 0, 0) / 8)
}
...
case class Ipv4(b1: Int, b2: Int, b3: Int, b4: Int) {
  ...
  if (b1 > 255 || b2 > 255 || b3 > 255 || b4 > 255 || b1 < 0 || b2 < 0 ||
      b3 < 0 || b4 < 0)
  ...
  override def equals(that: Any): Boolean = that match {
    case that: Ipv4 =>
      b1 == that.b1 && b2 == that.b2 && b3 == that.b3 && b4 == that.b4
    case _ => false
  }
...
class Subnet(baseIp: Ipv4, val bits: Int) extends Iterable[Ipv4] {
  if (bits < 0 || bits > 32)
    throw new InstantiationException(
        "The subnet size must be in the range 0-32")
