...
object HttpSupport {
  ...
  implicit def basicHttpSupport[H : UriCapable]: HttpSupport[H] =
    new HttpSupport[H] {
      ...
      def doHttp[C : PostType, T](
        ...
        mode wrap {
          ...
          val conn: URLConnection = new URL(
              implicitly[UriCapable[H]].uri(res).toString).openConnection()
          ...
          httpBasicAuthentication.credentials foreach {
            case (username, password) =>
              conn.setRequestProperty(
                  "Authorization",
                  "Basic " + NetUrl.base64
                    .encode(s"$username:$password".getBytes("UTF-8"))
                    .mkString)
          }
          ...
          new HttpResponse(mapAsScalaMap(conn.getHeaderFields()).toMap
                             .mapValues(_.to[List]),
                           statusCode,
                           is)
        }
