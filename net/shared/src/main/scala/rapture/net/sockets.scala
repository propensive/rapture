...
object Tcp {
  ...
  def listen[K](port: Int)(
      implicit ib: InputBuilder[InputStream, K],
      ob: OutputBuilder[OutputStream, K],
      mode: Mode[`Tcp.listen`]): mode.Wrap[(Input[K], Output[K]), Exception] =
    mode.wrap {
...
object Socket {
  def apply(hostname: String, port: Int): SocketUri =
    new SocketUri(hostname, port)
  def apply(hostname: String, svc: services.tcp.Port): SocketUri =
    new SocketUri(hostname, svc.portNo)
