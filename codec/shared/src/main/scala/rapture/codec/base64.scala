...
class Base64Codec[C <: CodecType](val char62: Char = '+',
  ...
  def encode(in: Array[Byte]): String = {
    ...
    if (inLen == 0) ""
      ...
      val outDataLen =
        if (endPadding) ((inLen - 1) / 3 + 1) << 2 else ((inLen << 2) + 2) / 3
      val outLen =
        if (lineBreaks) outDataLen + (outDataLen - 1) / 76 << 1 else outDataLen
      ...
      while (inPos < evenLen) {
        val block =
          (in(inPos) & 0xFF) << 16 | (in(inPos + 1) & 0xFF) << 8 |
          (in(inPos + 2) & 0xFF)
      ...
      if (left > 0) {
        ...
        val block =
          ((in(evenLen) & 0xFF) << 10) |
          (if (left == 2) (in(inLen - 1) & 0xFF) << 2 else 0)
  ...
  def decode(data: String): Either[Int, Array[Byte]] = {
    ...
    if (inLen == 0) Right(alloc(0))
      ...
      val padding =
        if (in(inLen - 1) == padChar) (if (in(inLen - 2) == padChar) 2 else 1)
        else 0

      // FIXME: This doesn't seem to accommodate different kinds of linebreak
      val lineBreaks =
        if (inLen > 76) (if (in(76) == '\r') inLen / 78 else 0) << 1 else 0
      ...
      while (outPos < evenLen) {
        ...
        val block =
          decodabet(in(inPos)) << 18 | decodabet(in(inPos + 1)) << 12 | decodabet(
              in(inPos + 2)) << 6 | decodabet(in(inPos + 3))
      ...
      if (outPos < outLen) {
        val block =
          decodabet(in(inPos)) << 18 | decodabet(in(inPos + 1)) << 12 |
          (if (inPos + 2 < inLen - padding) decodabet(in(inPos + 2)) << 6
           else 0)
