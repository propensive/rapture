...
trait LowPriorityImplicits extends LowerPriorityImplicits {
  ...
  implicit object FileStreamByteWriter extends Writer[FsUrl, Byte] {
    def output(url: FsUrl): Output[Byte] =
      new ByteOutput(
          new BufferedOutputStream(new FileOutputStream(url.javaFile)))
  }

  implicit object FileStreamByteAppender extends Appender[FsUrl, Byte] {
    def appendOutput(url: FsUrl): Output[Byte] =
      new ByteOutput(
          new BufferedOutputStream(new FileOutputStream(url.javaFile, true)))
  }
  ...
  implicit object NavigableFile extends Navigable[FsUrl] {
    def children(url: FsUrl)(implicit mode: Mode[`Navigable#children`]
    ): mode.Wrap[List[FsUrl], Exception] =
      mode.wrap {
        if (url.isFile) Nil else url.javaFile.list().to[List].map(url / _)
      }

    def isDirectory(url: FsUrl)(implicit mode: Mode[`Navigable#isDirectory`]
    ): mode.Wrap[Boolean, Exception] =
...
object `package` extends LowPriorityImplicits {
  ...
  implicit object FileStreamByteReader
      extends JavaInputStreamReader[FsUrl](
          f => new FileInputStream(f.javaFile))

  implicit class EnrichedFileUriContext(uc: UriContext.type) {
    def file(constants: List[String])(variables: List[String]) = {
      val fileUrl = constants
        .zip(variables :+ "")
        .map { case (a, b) => a + b }
        .mkString
        .split("/")
...
object FsUrl {
  implicit val fileCpUrl: ClasspathUrlable[FsUrl] =
    new ClasspathUrlable[FsUrl] {
      def toClasspathUrlItem(f: FsUrl) =
        ClasspathUrlItem(List(new java.net.URL(f.toString)))
    }
  ...
  implicit def fileSlashRelativePath[RP <: RelativePath]: Dereferenceable[
      FsUrl, RP, FsUrl] =
    new Dereferenceable[FsUrl, RP, FsUrl] {
      def dereference(p1: FsUrl, p2: RP) =
        FsUrl(p1.elements.dropRight(p2.ascent) ++ p2.elements)
    }

  implicit def fileSlashRootedPath[RP <: RootedPath]: Dereferenceable[
      FsUrl, RP, FsUrl] =
    new Dereferenceable[FsUrl, RP, FsUrl] {
      def dereference(p1: FsUrl, p2: RP) = {
        val start =
  ...
  implicit def fileParentable: Parentable[FsUrl, FsUrl] =
    new Parentable[FsUrl, FsUrl] {
...
case class FsUrl(val elements: Seq[String]) {
  ...
  lazy val javaFile: java.io.File =
    new java.io.File(this.uri.schemeSpecificPart.drop(2))
  ...
  def hidden: Boolean =
    if (exists) javaFile.isHidden() else throw new Exception()

  /** Returns the date of the last modification to the file or directory. */
  def lastModified[I : TimeSystem.ByInstant](
      implicit mode: Mode[FsMethods]): mode.Wrap[I, Exception] =
    mode.wrap(
        javaFile.lastModified() match {
  ...
  def mkdir(makeParents: Boolean = false)(
      implicit mode: Mode[FsMethods]): mode.Wrap[Boolean, Exception] =
    mode.wrap(if (makeParents) javaFile.mkdirs() else javaFile.mkdir())

  /** Extract the file extension from the name of this file. */
  def extension(
      implicit mode: Mode[FsMethods]): mode.Wrap[Option[String], Exception] =
    mode.wrap(
