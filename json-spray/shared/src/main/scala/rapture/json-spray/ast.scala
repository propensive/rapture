...
private[spray] object SprayAst extends JsonBufferAst {
  ...
  override def dereferenceObject(obj: Any, element: String): Any =
    obj match {
      case obj: JsObject =>
        try obj.getFields(element)(0) catch {
  ...
  def getArray(array: Any): List[Any] = array match {
    case v: JsValue =>
      try v.convertTo[List[JsValue]] catch {
        case e: Exception =>
          throw TypeMismatchException(getType(array), DataTypes.Array)
      }
    case _ => throw TypeMismatchException(getType(array), DataTypes.Array)
  }
  ...
  def getBoolean(boolean: Any): Boolean = boolean match {
    case v: JsValue =>
      try v.convertTo[Boolean] catch {
        case e: Exception =>
          throw TypeMismatchException(getType(boolean), DataTypes.Boolean)
      }
  ...
  def getBigDecimal(bigDecimal: Any): BigDecimal = bigDecimal match {
    case v: JsValue =>
      try v.convertTo[BigDecimal] catch {
        case e: Exception =>
  ...
  def getDouble(double: Any): Double = double match {
    case v: JsValue =>
      try v.convertTo[Double] catch {
        case e: Exception =>
          throw TypeMismatchException(getType(double), DataTypes.Number)
      }
  ...
  def getString(string: Any): String = string match {
    case v: JsValue =>
      try v.convertTo[String] catch {
        case e: Exception =>
          throw TypeMismatchException(getType(string), DataTypes.String)
      }
  ...
  def setObjectValue(obj: Any, name: String, value: Any): Any =
    (value, obj) match {
      case (value: JsValue, obj: JsValue) =>
        obj.asJsObject.fields.updated(name, value).toJson
    }
  ...
  def addArrayValue(array: Any, value: Any): Any = array match {
    case v: JsValue =>
      (v.convertTo[Array[JsValue]] :+ value.asInstanceOf[JsValue]).toJson
  }
  ...
  def fromArray(array: Seq[Any]): Any =
    array.map(_.asInstanceOf[JsValue]).toJson
