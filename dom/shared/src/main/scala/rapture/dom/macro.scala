...
object DomMacros {
  ...
  def reportElementErrorMacro[Child <: ElementType : c.WeakTypeTag,
                              ...
                              Att2 <: AttributeType : c.WeakTypeTag](
      c: WhiteboxContext)(
    ...
    if (weakTypeOf[Elem].weak_<:<(weakTypeOf[Child])) {
      ...
      val found = weakTypeOf[Elem].toString
        .split(" with ")
        .map(_.split("\\.").last)
        .to[Vector] match {
        case Vector(one) => one
      ...
      val expected = weakTypeOf[Child].toString
        .split(" with ")
        .map(_.split("\\.").last)
        .to[Vector] match {
        case Vector(one) => one
  ...
  def reportElementError2Macro[Child <: ElementType : c.WeakTypeTag,
    ...
    if (weakTypeOf[Elem].weak_<:<(weakTypeOf[Child])) {
      reify {
        value.splice
          .asInstanceOf[Element[_ <: ElementType, Child, _ <: AttributeType]]
      }
    } else {
      val found = weakTypeOf[Elem].toString
        .split(" with ")
