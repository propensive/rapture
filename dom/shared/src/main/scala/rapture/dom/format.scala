    ...
    implicit val domFormatterImplicit = new DomFormatter[String] {
      def format(element: DomNode[_, _, _]): String = element match {
        ...
        case elem: Element [_, _, _] =>
          ...
          val as =
            elem.tagName +: elem.attributes.to[List].filter(_._2 != null).map {
              case (k, v) =>
                k.name + "=\"" + k.serialize(v.asInstanceOf[k.Value]) + "\""
            }
...
object DomFormatter {
  implicit val domFormatterImplicit: DomFormatter[String] =
    new DomFormatter[String] {
      ...
      protected def format(
          indent: Int, element: DomNode[_, _, _]): Vector[(Int, String)] =
        element match {
          case TextNode(t) =>
          ...
          case elem: Element [_, _, _] =>
            ...
            val xs = children
              .foldLeft(Vector[(Int, String)]() -> false) {
                ...
                case ((acc, blk), child) =>
                  ...
                  if (join)
                    (acc.init :+ ((acc.last._1, acc.last._2 + cs(0)._2)),
                     !join)
                  else (acc ++ cs, !join)
              }
              ._1
            ...
            if (xs.isEmpty && !elem.forceClosingTag)
              Vector((indent -> s"<${as.mkString(" ")}/>"))
            else if (hasBlock || elem.block)
              (indent -> s"<${as.mkString(" ")}>") +: xs :+
              (indent -> s"</${elem.tagName}>")
            else
              Vector(
                  indent -> s"<${as.mkString(" ")}>${xs.map(_._2).mkString}</${elem.tagName}>")
...
trait DomFormatter[Output] {
  protected def text(string: String): String =
