...
object Applicable {
  implicit def reportElementError[
      Child <: ElementType, Att <: AttributeType, Child2 <: ElementType, Elem <: ElementType, Att2 <: AttributeType](
      value: DomNode[Child2, Elem, Att]): Any = macro DomMacros
    .reportElementErrorMacro[Child, Att, Child2, Elem, Att2]

  implicit def listToApplicable[
      Child <: ElementType, Elem <: ElementType, Att <: AttributeType, Ap <: Element[
          Child, Elem, Att]](apps: List[Ap]): Element[Child, Elem, Att] =
    apps.head

  implicit def wrapIterable[
      Child <: ElementType, This <: ElementType, Att <: AttributeType](
      xs: Iterable[Element[Child, This, Att]]): ElementLike[Child, This, Att] =
    ElementSeq[Child, This, Att](xs)
}

trait Applicable[+ChildType,
                 +ThisAttType,
                 AppliedType[
                     _ <: ElementType, _ <: ElementType, _ <: AttributeType]] {
  def application[
      Child <: ElementType, This <: ElementType, Att <: AttributeType](
    ...
    AppliedElement[Child, This, Att](
        ...
        applied
          ...
          .asInstanceOf[List[Element[
                      _ <: ElementType, Child, _ <: AttributeType]]],
...
case class DomNodes[
    ChildType <: ElementType, ThisType <: ElementType, AttType <: AttributeType](
    elements: Vector[Element[ChildType, ThisType, AttType]])
    extends AnyVal {

  def apply(i: Int = 0)(implicit mode: Mode[`DomNodes#apply`]) =
    mode.wrap(elements(i))
...
case class MissingAttributeException(name: String)
    extends Exception(s"The attribute $name was not found")

case class TextNode[
    ThisType <: ElementType, ThisAttType <: AttributeType, Position <: ElementType](
    text: String)
    extends DomNode[Position, ThisType, ThisAttType] {
  def format[Output](implicit formatter: DomFormatter[Output]): Output =
...
object Element {
  implicit def reportElementError2[
      Child <: ElementType, Att <: AttributeType, Child2 <: ElementType, Elem <: ElementType](
      value: DomNode[Child2, Elem, Att]): Any = macro DomMacros
    .reportElementError2Macro[Child, Att, Child2, Elem]
}

sealed abstract class ElementLike[ChildType <: ElementType,
                                  ThisType <: ElementType,
                                  AttType <: AttributeType]
    extends DomNode[ChildType, ThisType, AttType]
    with Applicable[ThisType, Nothing, AppliedElement] with Dynamic
    with Product with Serializable {}

sealed abstract class Element[ChildType <: ElementType,
                              ThisType <: ElementType,
                              AttType <: AttributeType]
  ...
  def selectDynamic(s: String)(implicit ar: AttributeKey[s.type, AttType],
                               mode: Mode[`Element#selectDynamic`]) =
    mode.wrap {
...
case class Tag[ChildType <: ElementType,
               ThisType <: ElementType,
               AttType <: AttributeType](
    forceClosingTag: Boolean = false, override val block: Boolean = true)(
    implicit assigned: AssignedName)
  ...
  def apply[AppliedType[_ <: ElementType,
                        _ <: ElementType,
...
case class ElementSeq[
    ChildType <: ElementType, ThisType <: ElementType, AttType <: AttributeType](
    elems: Iterable[Element[ChildType, ThisType, AttType]])
    extends ElementLike[ChildType, ThisType, AttType]

case class EmptyElement[ChildType <: ElementType,
                        ThisType <: ElementType,
                        AttType <: AttributeType](
    tagName: String,
  ...
  def apply(elements: DomNode[_ <: ElementType, ChildType, _ <: AttributeType]*
    ...
    AppliedElement[ChildType, ThisType, AttType](
        tagName, attributes, elements.to[List], forceClosingTag, block)
}

case class AppliedElement[ChildType <: ElementType,
                          ThisType <: ElementType,
                          AttType <: AttributeType](
    tagName: String,
...
@implicitNotFound(
    "Cannot access the attribute ${Name} on ${AttType} DOM nodes")
abstract class AttributeKey[+Name <: String, AttType <: AttributeType](
    val name: String, actualName: String = null) {
  type Value
  ...
  def set[Elem <: ElementType](v: Value) =
    new Attribute[Elem, AttType, Value](
        this.asInstanceOf[AttributeKey[String, AttributeType]], v)
...
object Attribute {
  def apply[Att <: AttributeType, V](
      name: String, actualName: String = null)(serializer: V => String
  ): AttributeKey[name.type, Att] { type Value = V } =
    new AttributeKey[name.type, Att](
        if (actualName == null) name else actualName) {
      type Value = V
...
class Attribute[Elem <: ElementType, AttType <: AttributeType, Value](
    val id: AttributeKey[String, AttributeType], val value: Value)
