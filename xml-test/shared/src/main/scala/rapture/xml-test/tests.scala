...
class StdlibTests()
    extends XmlTests(stdlib.implicitXmlAst, stdlib.implicitXmlStringParser)

class MutableStdlibTests()
    extends MutableXmlTests(
        stdlib.implicitXmlAst, stdlib.implicitXmlStringParser)

abstract class XmlTests(ast: XmlAst, parser: Parser[String, XmlAst])
    extends TestSuite {
  ...
  val `Extract missing value with case class default` =
    test {
      xml"""<obj><beta>0</beta></obj>""".as[HasDefault]
    } returns HasDefault("yes", 0)

  val `Extract missing value with case class default 2` =
    test {
...
abstract class MutableXmlTests(
    ast: XmlBufferAst, parser: Parser[String, XmlBufferAst])
    extends TestSuite {
