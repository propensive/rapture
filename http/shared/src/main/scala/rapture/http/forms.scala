...
object Forms extends Widgets with Parsers {
  ...
  trait FormValidation { this: (BasicForm with FormValidation) =>
    ...
    val validTwitterHandle: Option[String] => List[String] = {
      case Some(s) => if(s.matches("|[a-zA-Z0-9_]{1,15}")) Nil else List("Please enter a valid Twitter handle, or leave this field blank.")
      case None => Nil
    }
    
    val validEmailAddress: Option[String] => List[String] = {
      case Some(s) => if(s.matches("""^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2}[a-z]*)$""")) Nil else List("Please enter a valid email address")
      case None => Nil
    }
  ...
  abstract class WebForm[L](name: Symbol, params: Map[String, String] = Map(),
    ...
    implicit val checkboxRenderer = new Renderer[Boolean, Field[Boolean], Checkbox] {
      ...
      def render(f: Field[Boolean], w: Checkbox): RenderType =
        Label(
          Input(typ = "checkbox", htmlSyntax.value = "1", htmlSyntax.name = f.name,
    ...
    implicit def dropdownRenderer[T, Q] = new Renderer[T, Field[T], Dropdown[Q]] {
      def render(f: Field[T], w: Dropdown[Q]): RenderType =
        Select(htmlSyntax.name = f.name)(
          w.options.map { opt => Option(value = w.id(opt))(TextNode(w.description(opt))) }: _*
        )
    }
  ...
  class BootstrapForm[L: Linkable](name: Symbol, params: Map[String, String],
    ...
    def wrap[T, F <: Field[T], W <: Widget](field: F, widget: W)
      ...
      Div(classes = List.strap("control-group", field.validationIssues.headOption.map(x => "error")))(
        ...
        Div(classes = Seq("controls"))(
          ...
          Div(
            Div,
            field.validationIssues.map { i => Span(classes = Seq("help-inline"))(i+" ") }: _*
          )
        )
      )
    ...
    def submitRow = Div(Input(htmlSyntax.name = Symbol(formName+"_submit"), typ = "submit", classes = Seq("btn", "btn-primary"), value = submitButtonText))
  ...
  trait TabularLayout[L] { this: (WebForm[L] with TabularLayout[L]) =>
    ...
    def submitRow = if(hideLabels) Tr(Td, Td(submitButton)) else Tr(
      Td,
      Td,
      Td,
      Td(submitButton)
