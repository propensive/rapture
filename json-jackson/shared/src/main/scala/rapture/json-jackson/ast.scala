...
private[jackson] object JacksonAst extends JsonAst {
  ...
  def getDouble(number: Any): Double = number match {
    case number: JsonNode if number.isBigDecimal =>
      number.decimalValue.doubleValue
    case number: JsonNode if number.isBigInteger =>
      number.bigIntegerValue.doubleValue
  ...
  def getBigDecimal(number: Any): BigDecimal = number match {
    case number: JsonNode if number.isBigDecimal =>
      BigDecimal(number.decimalValue)
    case number: JsonNode if number.isBigInteger =>
      BigDecimal(number.bigIntegerValue)
    case number: JsonNode if number.isNumber => number.asDouble
  ...
  def setObjectValue(obj: JsonNode, name: String, value: JsonNode): Unit =
    obj match {
