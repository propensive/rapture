...
private[json] object JsonMacros {
  def jsonExtractorMacro[T : c.WeakTypeTag, Th](
      c: WhiteboxContext): c.Expr[Extractor[T, Json] { type Throws = Th }] =
  ...
  def jsonSerializerMacro[T : c.WeakTypeTag](c: WhiteboxContext)(
      ast: c.Expr[JsonAst]): c.Expr[Serializer[T, Json]] =
