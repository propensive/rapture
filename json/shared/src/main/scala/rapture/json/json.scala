...
private[json] trait Json_2 {
  implicit def jsonExtractorMacro[T <: Product, Th]: Extractor[T, Json] {
    type Throws = Th
  } = macro JsonMacros.jsonExtractorMacro[T, Th]

  implicit def jsonSerializerMacro[T <: Product](
      implicit ast: JsonAst): Serializer[T, Json] = macro JsonMacros
...
private[json] trait JsonDataCompanion[
    +Type <: JsonDataType[Type, AstType], AstType <: JsonAst]
  ...
  private[json] def doFormat(json: Any,
                             ln: Int,
                             ast: AstType,
                             pad: String = " ",
                             brk: String = "\n"): String = {
        ...
        if (ast.isString(j)) {
          ...
          if (keys.isEmpty) "{}"
            ...
            List("{", keys map { k =>
              ...
              s"""${indent}${pad}"${k}":${pad}${doFormat(
                  inner, ln + 1, ast, pad, brk)}"""
            } mkString s",${brk}", s"${indent}}") mkString brk
...
private[json] trait JsonDataType[
    +T <: JsonDataType[T, AstType], AstType <: JsonAst]
...
object JsonBuffer extends JsonDataCompanion[JsonBuffer, JsonBufferAst] {
  ...
  def construct(any: MutableCell, path: Vector[Either[Int, String]])(
      implicit ast: JsonBufferAst): JsonBuffer = new JsonBuffer(any, path)
}
...
object Json extends JsonDataCompanion[Json, JsonAst] with Json_1 {
  ...
  def construct(any: MutableCell, path: Vector[Either[Int, String]])(
      implicit ast: JsonAst): Json = new Json(any, path)

  def ast(json: Json): JsonAst = json.$ast

  def extractor[T](implicit ext: Extractor[T, Json]
      ): Extractor[T, Json] { type Throws = ext.Throws } = ext
  ...
  implicit def jsonCastExtractor[T : JsonCastExtractor](
    ...
    new Extractor[T, JsonDataType[_, _ <: JsonAst]] {
      ...
      def extract(
          value: JsonDataType[_, _ <: JsonAst],
          ast2: DataAst,
...
class JsonBuffer(
  ...
  def $deref(path: Vector[Either[Int, String]]): JsonBuffer =
    new JsonBuffer($root, path)
