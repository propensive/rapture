...
object JsonVerifier {
  ...
  case class VerifierException(
      strNo: Int, pos: Int, expected: String, found: Char)
      extends Exception
  case class DuplicateKeyException(strNo: Int, pos: Int, key: String)
      extends Exception
  ...
  def verify(parts: List[String]) = {
    ...
    def failPosition(expected: String) =
      throw VerifierException(n, i, expected, cur)
    def duplicateKey(start: Int, key: String) =
      throw DuplicateKeyException(n, start, key)
        ...
        case '\\' =>
          ...
          cur match {
            ...
            case 'u' =>
              consume('u')
              1 to 4 foreach { j =>
                if (cur.isDigit || cur >= 'a' && cur <= 'f' || cur >= 'A' &&
                    cur <= 'F') next()
