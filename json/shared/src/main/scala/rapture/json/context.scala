...
private[json] object JsonDataMacros extends DataContextMacros[Json, JsonAst] {
    ...
    try {
      ...
      case JsonValidator.ValidationException(strNo, pos, expected, found) =>
        ...
        Some(
            (strNo,
             pos,
             s"failed to parse Json literal: expected $expected, but found $f"))
  ...
  override def contextMacro(c: BlackboxContext)(
      exprs: c.Expr[ForcedConversion[Json]]*)(
      parser: c.Expr[Parser[String, JsonAst]]): c.Expr[Json] =
    super.contextMacro(c)(exprs: _*)(parser)
}

private[json] object JsonBufferDataMacros
    extends DataContextMacros[JsonBuffer, JsonBufferAst] {

  def companion(
      c: BlackboxContext): c.Expr[DataCompanion[JsonBuffer, JsonBufferAst]] =
  ...
  override def contextMacro(c: BlackboxContext)(
      exprs: c.Expr[ForcedConversion[JsonBuffer]]*)(
...
private[json] class JsonStrings(sc: StringContext) {
  class JsonContext() extends DataContext(Json, sc) {
    def apply(exprs: ForcedConversion[Json]*)(
        implicit parser: Parser[String, JsonAst]): Json = macro JsonDataMacros.contextMacro
  }
...
private[json] class JsonBufferStrings(sc: StringContext) {
  class JsonBufferContext() extends DataContext(JsonBuffer, sc) {
    def apply(exprs: ForcedConversion[JsonBuffer]*)(
        implicit parser: Parser[String, JsonBufferAst]): JsonBuffer = macro JsonBufferDataMacros.contextMacro
  }
