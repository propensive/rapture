...
case class Opts[T: Optable](options: T*) {
  ...
  def suggester: Suggester = new Suggester {
    override def suggest(prefix: String): Suggestions =
      Suggestions(SuggestionGroup(None, options.to[Vector].map { opt =>
...
case class Param[+T: ParamParser](longName: String = null, shortName: String = null,
    description: String = null, suggester: Suggester = NoSuggestions,
    repeatable: Boolean = false) {
  def parse(s: List[String]): Option[T] = implicitly[ParamParser[T]].parse(s)
}

trait ParamParser_1 {
  implicit val intParamParser: ParamParser[Int] =
    new ParamParser[Int] { def parse(s: List[String]) = s.headOption.map(_.toInt) }
  
  implicit val booleanParamParser: ParamParser[Boolean] =
    new ParamParser[Boolean] { def parse(s: List[String]): Option[Boolean] = Some(true) }
}
  ...
  def suggester(exclusions: Set[String]): Suggester = new Suggester {
    override def suggest(prefix: String): Suggestions = Suggestions(
      ...
      SuggestionGroup(None, options.filterNot { opt =>
        exclusions.contains(paramable.longName(opt).drop(2)) ||
            exclusions.contains(paramable.shortName(opt).drop(1))
      }.map { opt => paramable.shortName(opt) -> Vector() }.filter(_._1 startsWith prefix).toMap, true)
    )
  }
...
case class ParamMap(params: Map[String, List[Arg]], completer: Option[Completer]) {
  ...
  def get[T, P[_]](param: P[T], suggesters: Suggester*)(implicit paramable: Paramable[T, P]): Option[T] = {
    params.find { case (k, v) =>
      k == implicitly[Paramable[T, P]].longName(param).substring(2) || k == implicitly[Paramable[T, P]].shortName(param).substring(1)
    }.flatMap { case (_, args) =>
      val values = suggesters padTo (args.length, NoSuggestions) zip args map { case (suggester, arg) => arg(suggester) }
      paramable.parse(param, values.to[List])
    }
  }

  def apply[T, P[_]](param: P[T], suggesters: Suggester*)(implicit paramable: Paramable[T, P]): T =
    get(param, suggesters: _*).getOrElse { if(completer.isDefined) null.asInstanceOf[T] else throw ParamGetException(paramable.longName(param)) }
}
...
case class CmdLine(pwd: FsUrl, params: Vector[Arg], completer: Option[Completer]) extends
  ...
  override def toString = params.map {
...
object Suggestions {
  def from[T](it: Iterable[T])(fn: T => String, fns: (T => Any)*) = new Suggester {
    override def suggest(prefix: String) = Suggestions(SuggestionGroup(None,
      it.map { e => fn(e) -> ((if(fns.isEmpty) "" else "--") +: fns.to[Vector].map(_(e).toString)) }.filter(_._1.startsWith(prefix)).toMap, false
    ))
  }
