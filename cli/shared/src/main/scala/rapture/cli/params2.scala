...
object New {
  ...
  case class ParamMap(args: String*) {
    ...
    @tailrec
    private def parseArgs(gs: List[Parameter] = Nil,
                          n: Int = 0,
                          off: Int = 0): List[Parameter] = {
      if (n == args.length) gs
      else if (args(n) startsWith "--") {
        val idx = args(n).indexOf('=')
        if (idx < off)
          parseArgs(Parameter(Part(n, 2, args(n).length)) :: gs, n + 1)
        else parseArgs(Parameter(Part(n, 2, idx)) :: gs, n, idx + 1)
      } else if (args(n) startsWith "-") {
        if (off == 0) parseArgs(gs, n, 1)
        else if (args(n).length == off + 1)
          parseArgs(Parameter(Part(n, off, off + 1)) :: gs, n + 1)
        else parseArgs(Parameter(Part(n, off, off + 1)) :: gs, n, off + 1)
      } else {
        if (gs.isEmpty) parseArgs(gs, n + 1)
        else
          parseArgs(gs.head.copy(values = gs.head.values :+ Part(
                              n, 0, args(n).length)) :: gs.tail,
                    n + 1)
      }
    }
  ...
  case class MissingParam(name: String)
      extends ParamException(s"the parameter --$name was missing")

  case class InvalidValue(value: String, name: String)
      extends ParamException(
          s"the value '$value' is not valid for the parameter --$name")

  case class UnexpectedParam(param: String)
      extends ParamException(s"found unexpected parameter '$param'")
  ...
  trait Construct[-A <: Params, -B <: Params] { construct =>
    ...
    def swap: Construct[B, A] {
      type And = construct.And; type Or = construct.Or
    } =
      new Construct[B, A] {
  ...
  trait Construct_1 {
    ...
    implicit def general[A <: Params, B <: Params]: Construct[A, B] {
      type And = A with B; type Or = A with B
    } = {
  ...
  object Construct extends Construct_1 {
    ...
    implicit def leftProduct[A <: Params, B <: SimpleParam[_]]: Construct[
        ProductParams[A], B] {
      ...
      new Construct[ProductParams[A], B] {
        ...
        def and(a: ProductParams[A], b: B) =
          ProductParams[A with B](a.elements + b)
    ...
    implicit def rightProduct[A <: SimpleParam[_], B <: Params]: Construct[
        A, ProductParams[B]] {
      type And = B with A; type Or = ProductParams[B] with A
    } = leftProduct[B, A].swap

    implicit def leftCoproduct[A <: Params, B <: SimpleParam[_]]: Construct[
        CoproductParams[A], B] {
      type And = CoproductParams[A] with B; type Or = A with B
    } = {
      ...
      new Construct[CoproductParams[A], B] {
        ...
        def or(a: CoproductParams[A], b: B) =
          CoproductParams[A with B](a.elements :+ b)
      }
    }

    implicit def rightCoproduct[A <: SimpleParam[_], B <: Params]: Construct[
        A, CoproductParams[B]] {
      type And = CoproductParams[B] with A; type Or = B with A
    } =
  ...
  trait Params { params =>
    ...
    def parse(args: ParamMap, tabArg: Int = -1)(
        implicit suggestOutput: SuggestionOutput,
        mode: Mode[`Params.parse`]): mode.Wrap[Result, ParamException] =
      mode.wrap {

        val (result, lastArgs, ss) =
          check(ParamUsage(args, Set()), mode, tabArg, Suggestions(None))
    ...
    def check(args: ParamUsage,
              mode: Mode[_],
              tabArg: Int,
              ss: Suggestions): (Result, ParamUsage, Suggestions)
  ...
  case class ProductParams[Ps <: Params](elements: Set[Params])
      extends Params {
    ...
    def check(args: ParamUsage,
      ...
      val (finalArgs, finalElems, newSs) =
        elements.foldLeft((args, Set[(Params, Any)](), ss)) {
          case ((args, es, ss), key) =>
  ...
  case class CoproductParams[Ps <: Params](elements: Vector[Params])
      extends Params {
    ...
    def check(args: ParamUsage,
      ...
      elems match {
        case (key, (res, args, newSs)) :: Nil =>
          (Coproduct[CoproductTypes](key -> res), args, newSs)
        case Nil => mode.exception(MissingParam(toString))
        case _ :: (key, _) :: _ =>
          mode.exception(UnexpectedParam(key.toString))
      }
    }
  ...
  object ToSuggestion {
    ...
    implicit val stringSuggestion: ToSuggestion[String] =
  ...
  case class SimpleParam[T : Param.Extractor](val keys: Vector[String])
      extends Params { simpleParam =>
    ...
    def suggest(suggestions: T*)(
        implicit sug: ToSuggestion[T]): SimpleParam[T] =
      suggest { s =>
        suggestions
      }

    def suggest(suggestions: String => Seq[T])(
        implicit sug: ToSuggestion[T]): SimpleParam[T] = {
      val ss = suggestions
      new SimpleParam[T](keys) {
        override def toSuggestion(value: T): Vector[String] =
    ...
    def filter(fn: T => Boolean): SimpleParam[T] = new SimpleParam[T](keys) {
      override def toSuggestion(value: T): Vector[String] =
        simpleParam.toSuggestion(value)
      override def suggestions(str: String) =
        simpleParam.suggestions(str).filter(fn)
      override def checkValue = simpleParam.checkValue
    }
    ...
    def check(args: ParamUsage,
      ...
      val parameter =
        args.map(keys) getOrElse mode.exception(MissingParam(keys.head))

      val res =
        extractor.extract(parameter.values.map(_ ())) getOrElse {
          mode.exception(
              InvalidValue(parameter.key(), parameter.values.mkString(" ")))
        }
  ...
  object Param {
    ...
    object Extractor {
      implicit val stringExtractor: Extractor[String] = new Extractor[String] {
        ...
        def extract(values: Vector[String]): Option[String] =
      ...
      implicit val intExtractor: Extractor[Int] = new Extractor[Int] {
        def extract(values: Vector[String]): Option[Int] = values match {
          case Vector(v) =>
            try Some(v.toInt) catch { case e: Exception => None }
          case _ => None
        }
      }
    ...
    def apply[T : Extractor](
        shortName: Char, longName: Symbol): SimpleParam[T] =
    ...
    def apply[T : Extractor](longName: Symbol): SimpleParam[T] =
      alloc(Vector(longName.name))

    def flag(shortName: Char, longName: Symbol): SimpleParam[Unit] =
      alloc(Vector(shortName.toString, longName.name))

    def flag(shortName: Char): SimpleParam[Unit] =
      alloc(Vector(shortName.toString))
    def flag(longName: Symbol): SimpleParam[Unit] =
      alloc(Vector(longName.name))
  }
  ...
  object ProductContainsParam extends ProductContainsParam_1 {
    implicit def optional[
        V <: OptionParams[_ <: Params], P <: Params]: ProductContainsParam[
        V, P] = null
  }
  ...
  case class Product[T <: Params](tmap: Map[Params, Any]) {
    def apply[V <: Params](value: V)(
        implicit acc: ProductContainsParam[value.type, T]): value.Result =
      tmap(value).asInstanceOf[value.Result]

    override def toString =
      tmap.map { case (k, v) => s"$k: $v" }.mkString(", ")
  }
