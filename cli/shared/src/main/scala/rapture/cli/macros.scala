...
private[cli] object CliMacros {
  ...
  def shImplementation(c: BlackboxContext)(
      content: c.Expr[ShParam]*): c.Expr[Process] = {
    import c.universe._

    val params = c.prefix.tree match {
      case Apply(_, List(Apply(_, rawParts))) =>
        val parts =
          rawParts.to[Vector].zip(content.map(_.tree).to[Vector]).flatMap {
            case (x, y) => Vector(x, y)
          } :+ rawParts.last
        ...
        def add(chr: Char) = {
          param = if (param.isEmpty) Vector(Left(chr.toString))
          else
            param.last match {
              case Right(_) =>
        ...
        if (singleQuoted || doubleQuoted)
          c.abort(c.enclosingPosition, "unclosed quoted parameter")
        if (params.isEmpty)
          c.abort(c.enclosingPosition, "no command specified")
