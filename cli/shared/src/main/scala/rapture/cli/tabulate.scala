...
object Tabulation {
  ...
  def tabulate[C[E] <: Seq[E], T](
      collection: Seq[T], titles: Option[Seq[Col]] = None)(
    ...
    val widths = contentsWithTitles.map { _.map(_.content.length) }
      .reduce(_ zip _ map { case (a, b) => a max b })

    contentsWithTitles.map(
        _.zip(widths).map {
