...
abstract class AesEncryption {
  ...
  def encrypt(clearText: Array[Byte], iv: Array[Byte] = null): Array[Byte] = {
    ...
    if (iv == null) cipher.init(javax.crypto.Cipher.ENCRYPT_MODE, keySpec)
      ...
      cipher.init(
          javax.crypto.Cipher.ENCRYPT_MODE, keySpec, new IvParameterSpec(iv))

    val digest = Hash.digest[Sha256](clearText).bytes
    val paddedLength = (clearText.length >> 4) + 1 << 4
    val cipherText =
      new Array[Byte](paddedLength + (if (iv == null) 48 else 0))
    ...
    cipher.doFinal(
        clearText, 0, clearText.length, cipherText, if (iv == null) 48 else 0)
  ...
  def decrypt(cipherText: Array[Byte], iv: Array[Byte] = null)(
    ...
    val cipher = javax.crypto.Cipher.getInstance("AES/CBC/PKCS5Padding")
    val ips =
      if (iv == null) new IvParameterSpec(cipherText, 0, 16)
    ...
    val digest1 =
      if (iv == null) cipher.update(cipherText, 16, 48) else Array[Byte]()
