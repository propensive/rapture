...
trait DataCompanion[+Type <: DataType[Type, DataAst], -AstType <: DataAst] {
  ...
  def construct(any: MutableCell, path: Vector[Either[Int, String]])(
      implicit ast: AstType): Type

  def parse[Source : StringSerializer](s: Source)(
      implicit mode: Mode[ParseMethodConstraint],
      parser: Parser[Source, AstType]): mode.Wrap[Type, ParseException] =
    mode wrap {
      construct(try MutableCell(parser.parse(s).get) catch {
        case e: NoSuchElementException =>
          mode.exception(ParseException(String(s)))
      }, Vector())(parser.ast)
    }
  ...
  def format[T <: DataType[T, AstType]](data: T)(
      implicit f: Formatter[_ <: AstType]): f.Out =
    f.format(data.$normalize)
}

case class DynamicApplication[D](
    path: List[Either[Int, String]], application: ForcedConversion2[D])
...
case class DynamicPath[D](path: List[Either[Int, String]]) extends Dynamic {
  ...
  def applyDynamic(v: String)(i: Int) =
    DynamicPath[D](Left(i) :: Right(v) :: path)
...
trait DynamicData[+T <: DynamicData[T, AstType], +AstType <: DataAst]
    extends Dynamic {

  /** Assumes the Json object wraps a `Map`, and extracts the element `key`. */
  def selectDynamic(key: String): T = $deref(Right(key) +: $path)

  def applyDynamic(key: String)(i: Int = 0): T =
    $deref(Left(i) +: Right(key) +: $path)
...
object DataType {
  ...
  class DataClassOperations[T <: DataType[T, AstType], AstType <: DataAst](
      dataType: T) {
    def ++[S <: DataType[S, Rep] forSome { type Rep }](b: S): T = {
      ...
      def merge(a: Any, b: Any): Any = {
        if (ast.isObject(a) && ast.isObject(b)) {
          ast.fromObject(
              ast
                .getKeys(b)
                .foldLeft(ast.getObject(a)) {
              case (as, k) =>
                as +
                (k -> {
                      if (as contains k)
                        merge(as(k), ast.dereferenceObject(b, k))
                      else ast.dereferenceObject(b, k)
                    })
            })
        } else if (ast.isArray(a) && ast.isArray(b))
          ast.fromArray(ast.getArray(a) ++ ast.getArray(b))
        else b
      }
      ...
      val right =
        if (ast != b.$ast)
          ast.convert(b.$normalize, b.$ast.asInstanceOf[DataAst])
        else b.$normalize

      dataType.$wrap(merge(left, right), Vector())
    }

    def copy(pvs: (DynamicPath[T] => DynamicApplication[
            _ <: DataType[T, _ <: AstType]])*): T = {
            ...
            def nav(path: List[Either[Int, String]], dest: Any, v: Any): Any =
              path match {
                ...
                case Right(next) :: list =>
                  val d = try ast.dereferenceObject(dest, next) catch {
                    case e: Exception => ast.fromObject(Map())
                  }
                  val src =
                    ast.getObject(if (ast.isObject(dest)) dest else Map())
                  ast.fromObject(src + ((next, nav(list, d, v))))

                case Left(next) :: list =>
                  val d = try ast.dereferenceArray(dest, next) catch {
                    case e: Exception => ast.fromArray(List())
                  }
                  val src = if (ast.isArray(dest)) ast.getArray(dest) else Nil
                  ast.fromArray(src
                        .padTo(next + 1, ast.fromObject(Map()))
                        .updated(next, nav(list, d, v)))
              }
...
trait DataType[+T <: DataType[T, AstType], +AstType <: DataAst] {
  ...
  def \\(key: String): T =
    $wrap($ast.fromArray(derefRecursive(key, $normalize)))
        ...
        case (j, t :+ e) =>
          fn(({
            if (e.bimap(x => $ast.isArray(j), x => $ast.isObject(j))) {
              try e.bimap($ast.dereferenceArray(j, _),
                          $ast.dereferenceObject(j, _)) catch {
                case TypeMismatchException(exp, fnd) =>
                  throw TypeMismatchException(exp, fnd)
                case e: Exception =>
  ...
  def as[S](implicit ext: Extractor[S, T],
            mode: Mode[`Data#as`]): mode.Wrap[S, ext.Throws] =
...
trait MutableDataType[+T <: DataType[T, AstType], AstType <: MutableDataAst]
  ...
  def $updateParents(p: Vector[Either[Int, String]], newVal: Any): Unit =
    p match {
      ...
      case Left(idx) +: init =>
        val jb = $deref(init)
        val newJb = $ast.setArrayValue(
            Try(jb.$normalize).getOrElse($ast.fromArray(Nil)), idx, newVal)
      ...
      case Right(key) +: init =>
        val jb = $deref(init)
        val newJb = $ast.setObjectValue(
            Try(jb.$normalize).getOrElse($ast.fromObject(Map())), key, newVal)
  ...
  def updateDynamic(key: String)(v: ForcedConversion2[T]): Unit =
    if (!v.nothing)
      $updateParents(
          $path,
          $ast.setObjectValue(
              Try($normalize).getOrElse($ast.fromObject(Map())), key, v.value))

  /** Updates the `i`th element of the JSON array with the value `v` */
  def update[T2](i: Int, v: T2)(implicit ser: Serializer[T2, T]): Unit =
    $updateParents(
        $path,
        $ast.setArrayValue(Try($normalize).getOrElse($ast.fromArray(Nil)),
                           i,
                           ser.serialize(v)))

  /** Removes the specified key from the JSON object */
  def -=(k: String): Unit =
    $updateParents($path, $ast.removeObjectValue(doNormalize(true), k))
...
object ForcedConversion2 extends ForcedConversion2_1 {
  implicit def forceOptConversion[T, D](opt: Option[T])(
      implicit ser: Serializer[T, D]) =
    opt.map(t => ForcedConversion2[D](ser.serialize(t), false)) getOrElse ForcedConversion2[
        D](null, true)
}
...
object ForcedConversion extends ForcedConversion_1 {
  implicit def forceOptConversion[T, D](opt: Option[T])(
      implicit ser: Serializer[T, D]) =
    opt.map(t => ForcedConversion[D](ser.serialize(t), false)) getOrElse ForcedConversion[
        D](null, true)
}
...
trait ForcedConversion_2 {
  ...
  implicit def forceStringConversion[D, T : StringSerializer](value: T)(
      implicit ser: Serializer[String, D]) =
    ForcedConversion[D](
        ser.serialize(?[StringSerializer[T]].serialize(value)), false)
}

case class ForcedConversion[-D](value: Any, nothing: Boolean)

case class ParseException(
    source: String, line: Option[Int] = None, column: Option[Int] = None)
