...
object patternMatching {
  object exactArrays {
    implicit val implicitArrayMachingConfig = new ArrayMatchingConfig {
      def checkLengths = true
    }
  }

  object exactObjects {
    implicit val implicitArrayMachingConfig = new ObjectMatchingConfig {
      def checkSizes = true
    }
  }
...
object ArrayMatchingConfig {
  implicit val ignoreByDefault = new ArrayMatchingConfig {
    def checkLengths = false
  }
}

object ObjectMatchingConfig {
  implicit val ignoreByDefault = new ObjectMatchingConfig {
    def checkSizes = false
  }
}
...
abstract class DataContextMacros[
    +Data <: DataType[Data, DataAst], -AstType <: DataAst] {

  def parseSource(
      s: List[String], stringsUsed: List[Boolean]): Option[(Int, Int, String)]
  ...
  def contextMacro(c: BlackboxContext)(exprs: c.Expr[ForcedConversion[Data]]*)(
    ...
    c.prefix.tree match {
      case Select(Apply(Apply(_, List(Apply(_, rawParts))), _), _) =>
        ...
        val text =
          rawParts map { case lit @ Literal(Constant(part: String)) => part }
        ...
        val stringsUsed: List[Boolean] = (listExprs.tree match {
          case Apply(_, bs) =>
            bs.map {
              case Apply(Apply(TypeApply(Select(_, nme), _), _), _) =>
                nme.toString == "forceStringConversion"
            }
        })
        ...
        reify {
          ...
          val expressions: Iterator[ForcedConversion[_]] =
            listExprs.splice.iterator

          sb.append(textParts.next())

          while (textParts.hasNext) {
            sb.append(comp.splice
                  .construct(MutableCell(expressions.next.value), Vector())(
                      parser.splice.ast)
                  .toString)
            sb.append(textParts.next)
          }

          comp.splice.construct(
              MutableCell(parser.splice.parse(sb.toString).get),
              Vector())(parser.splice.ast)
        }
    }
  }
}
...
class DataContext[+Data <: DataType[Data, DataAst], -AstType <: DataAst](
  ...
  def unapplySeq[D <: DataType[D, DataAst]](data: D)(
    ...
    try {
      ...
      val txt = sc.parts.reduceLeft(
          _ + s""""${placeholder}${count.next()}${placeholder}" """ + _)

      val paths: Array[Vector[Either[Int, String]]] =
        Array.fill[Vector[Either[Int, String]]](sc.parts.length - 1)(Vector())

      val arrayLengths =
        new collection.mutable.HashMap[Vector[Either[Int, String]], Int]
      val objectSizes =
        new collection.mutable.HashMap[Vector[Either[Int, String]], Int]
      ...
      def extract(any: Any, path: Vector[Either[Int, String]]): Unit = {
        ...
        if (parser.ast.isScalar(any)) {
          ...
          if (data.$extract(path).as[Any] != any)
          ...
          obj foreach {
            case (k, v) =>
              if (parser.ast.isString(v))
                parser.ast.getString(v) match {
          ...
          array.zipWithIndex foreach {
            case (e, i) =>
              if (parser.ast.isString(e))
                parser.ast.getString(e) match {
                  case str: CharSequence =>
                    str match {
                      case PlaceholderNumber(n) =>
      ...
      val matchedObjectSizes = objectSizes.forall {
        case (p, s) =>
          if (objectMatching.checkSizes)
            parser.ast.getObject(data.$extract(p).$normalize).size == s
          else parser.ast.getObject(data.$extract(p).$normalize).size >= 0
      }

      if (extracts.exists(_.$root.value == null) || !matchedArrayLengths ||
          !matchedObjectSizes) None else Some(extracts)
    } catch {
      case e: Exception =>
        None
    }
