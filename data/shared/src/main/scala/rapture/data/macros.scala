...
object Macros {
  ...
  def extractorMacro[T : c.WeakTypeTag, Data : c.WeakTypeTag, Th](
    ...
    if (weakTypeOf[T] <:< typeOf[AnyVal]) {
      val param = paramLists(c)(
          declarations(c)(weakTypeOf[T]).collect {
        case t: MethodSymbol => t.asMethod
      }.find(_.isPrimaryConstructor).get).head.head

      val paramType = param.typeSignature

      val inferredExtractor = c.inferImplicitValue(
          appliedType(extractor, List(paramType, weakTypeOf[Data])),
          false,
          false)
      ...
      val defaults =
        weakTypeOf[T].typeSymbol.companionSymbol.typeSignature.declarations
          .to[List]
      ...
      val params =
        declarations(c)(weakTypeOf[T]).collect {
          ...
          case (p, idx) =>
            val deref = q"""data.selectDynamic(${Literal(
                Constant(p.name.decodedName.toString))})"""

            val NothingType = weakTypeOf[Nothing]

            val imp = try c.inferImplicitValue(
                appliedType(extractor, List(p.returnType, weakTypeOf[Data])),
                false,
                false) catch {

              case e: Exception =>
                implicitSearchFailures(p.returnType.toString) ::=
            ...
            if (defaults.contains(idx + 1))
              q"""
          mode.unwrap(if($deref.is($imp)) $deref.as($imp, mode.generic) else mode.wrap(${companionRef(
                  weakTypeOf[T])}.${termName(c, "apply$default$" + (idx + 1))}.asInstanceOf[${p.returnType}]), ${Literal(
                  Constant("." + p.name.decodedName))})
        """
            else
              q"""
          mode.unwrap($deref.as($imp, mode.generic), ${Literal(
                  Constant("." + p.name.decodedName))})
        """
        }
      ...
      if (!implicitSearchFailures.isEmpty) {
        ...
        val plural =
          ...
          else s"a ${weakTypeOf[Data].typeSymbol.name} extractor"

        val err =
          s"Could not generate a ${weakTypeOf[Data].typeSymbol.name} extractor for case " +
      ...
      c.Expr(
        ...
        }).asInstanceOf[_root_.rapture.data.Extractor[${weakTypeOf[T]}, ${weakTypeOf[
          Data]}] {
          type Throws = ${normalize(c)(
          typeIntersection(c)(throwsTypes.to[List]))}
        }]
      """)
    }
  }
  ...
  def serializerMacro[
      T : c.WeakTypeTag, Data : c.WeakTypeTag](c: WhiteboxContext)(
    ...
    val serializer =
      typeOf[Serializer[_, _]].typeSymbol.asType.toTypeConstructor
    ...
    if (weakTypeOf[T] <:< typeOf[AnyVal]) {
      ...
      val paramType = param.typeSignature
      val inferredSerializer = c.inferImplicitValue(
          appliedType(serializer, List(paramType, weakTypeOf[Data])),
          false,
          false)
      ...
      reify(
        ...
        def serialize(t: T): Any =
          ast.splice.fromObject(construction.splice.filterNot { v =>
