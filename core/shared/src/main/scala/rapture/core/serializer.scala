...
package integerFormats {
  ...
  object to1sf {
    def apply() = implicitIntegerFormat
    implicit val implicitIntegerFormat: IntegerFormat =
      IntegerSignificantFigures(1)
  }
  object to2sf {
    def apply() = implicitIntegerFormat
    implicit val implicitIntegerFormat: IntegerFormat =
      IntegerSignificantFigures(2)
  }
  object to3sf {
    def apply() = implicitIntegerFormat
    implicit val implicitIntegerFormat: IntegerFormat =
      IntegerSignificantFigures(3)
  }
  object to4sf {
    def apply() = implicitIntegerFormat
    implicit val implicitIntegerFormat: IntegerFormat =
      IntegerSignificantFigures(4)
  }
  object to5sf {
    def apply() = implicitIntegerFormat
    implicit val implicitIntegerFormat: IntegerFormat =
      IntegerSignificantFigures(5)
  }
  object to6sf {
    def apply() = implicitIntegerFormat
    implicit val implicitIntegerFormat: IntegerFormat =
      IntegerSignificantFigures(6)
  }
}
...
package booleanRepresentations {
  object trueFalse {
    ...
    implicit val implicitBooleanRepresentation: BooleanRepresentation =
      BooleanRepresentation("true", "false")
  }

  object digital {
    def apply() = implicitBooleanRepresentation
    implicit val implicitBooleanRepresentation: BooleanRepresentation =
      BooleanRepresentation("1", "0")
  }
  ...
  object onOff {
    def apply() = implicitBooleanRepresentation
    implicit val implicitBooleanRepresentation: BooleanRepresentation =
      BooleanRepresentation("on", "off")
  }
}

object BooleanRepresentation {
  implicit val defaultBooleanRepresentation: BooleanRepresentation =
    BooleanRepresentation("true", "false")
}
...
object DecimalFormat {
  implicit val defaultRounding: DecimalFormat = SignificantFigures(4)
}
...
case class DecimalPlaces(n: Int) extends DecimalFormat {
  def format(bigDecimal: BigDecimal): String = {
    ...
    bigDecimal
      .round(new java.math.MathContext(integral + n))
      .setScale(n)
      .toString
  }
}
...
case class SignificantFigures(n: Int) extends DecimalFormat {
  def format(bigDecimal: BigDecimal) =
    bigDecimal.round(new java.math.MathContext(n)).toString
}
...
object IntegerFormat {
  implicit val defaultRounding: IntegerFormat = ExactInteger
}
...
object StringSerializer {
  implicit def booleanSerializer(
      implicit bs: BooleanRepresentation): StringSerializer[Boolean] =
    new StringSerializer[Boolean] {
      def serialize(s: Boolean): String =
        if (s) bs.trueValue else bs.falseValue
    }

  implicit val charSerializer: StringSerializer[Char] =
    new StringSerializer[Char] {
      def serialize(s: Char): String = s.toString
    }

  implicit def byteSerializer(
      implicit df: IntegerFormat): StringSerializer[Byte] =
    new StringSerializer[Byte] {
      def serialize(s: Byte): String = df.format(BigInt(s))
    }

  implicit def shortSerializer(
      implicit df: IntegerFormat): StringSerializer[Short] =
    new StringSerializer[Short] {
      def serialize(s: Short): String = df.format(BigInt(s))
    }

  implicit def longSerializer(
      implicit df: IntegerFormat): StringSerializer[Long] =
    new StringSerializer[Long] {
      def serialize(s: Long): String = df.format(BigInt(s))
    }

  implicit def intSerializer(
      implicit df: IntegerFormat): StringSerializer[Int] =
    new StringSerializer[Int] {
      def serialize(s: Int): String = df.format(BigInt(s))
    }

  implicit val stringSerializer: StringSerializer[String] =
    new StringSerializer[String] { def serialize(s: String): String = s }

  implicit def doubleSerializer(
      implicit df: DecimalFormat): StringSerializer[Double] =
    new StringSerializer[Double] {
      def serialize(s: Double): String = df.format(BigDecimal(s))
    }

  implicit def floatSerializer(
      implicit df: DecimalFormat): StringSerializer[Float] =
    new StringSerializer[Float] {
      def serialize(f: Float): String = df.format(BigDecimal(f.toDouble))
    }

  implicit def bigDecimalSerializer(
      implicit df: DecimalFormat): StringSerializer[BigDecimal] =
    new StringSerializer[BigDecimal] {
      def serialize(s: BigDecimal): String = df.format(s)
    }

  implicit def bigIntSerializer(
      implicit df: IntegerFormat): StringSerializer[BigInt] =
    new StringSerializer[BigInt] {
      def serialize(s: BigInt): String = df.format(s)
    }
}
...
object String {
  ...
  def vauleOf(x: Array[Char], a: Int, b: Int): String =
    java.lang.String.valueOf(x, a, b)
