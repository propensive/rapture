...
private[core] object CoreMacros {
  def enumerateMacro[Cls : c.WeakTypeTag, T : c.WeakTypeTag](
      c: BlackboxContext)(value: c.Expr[Cls]): c.Expr[List[T]] = {
    ...
    val allMethods =
      weakTypeOf[Cls].members.to[List].filter(_.isMethod).map(_.asMethod)
    val matchingMethods =
      allMethods filter { m =>
  ...
  def assignedMethodNameMacro(c: BlackboxContext): c.Expr[MethodName] = {
    ...
    val name = c.enclosingClass.find {
      case DefDef(_, name, _, _, _, rhs) =>
        rhs.pos.isDefined && rhs.pos.point == c.macroApplication.pos.point
      case _ => false
    }.map {
    ...
    name getOrElse c.abort(
        c.enclosingPosition,
        "This method invocation must be assigned to a named identifier.")
  }
    ...
    val name =
      c.enclosingClass find {
        ...
        case ValDef(_, name, _, _) =>
          val dec =
            c.Expr[String](Literal(Constant(name.decodedName.toString)))
  ...
  def allocMacro[T : c.WeakTypeTag](c: BlackboxContext): c.Expr[Alloc0[T]] = {
    ...
    val construction = c.Expr[T](
        Apply(Select(New(TypeTree(weakTypeOf[T])), constructor(c)), List()))
    reify { new Alloc0[T] { def instantiate(): T = construction.splice } }
  }

  def allocMacro1[T : c.WeakTypeTag, P1 : c.WeakTypeTag](
      c: BlackboxContext): c.Expr[Alloc1[T, P1]] = {
    import c.universe._
    import compatibility._

    val construction =
      c.Expr[T](Apply(Select(New(TypeTree(weakTypeOf[T])), constructor(c)),
                      List(Ident(termName(c, "p1")))))

    reify {
      new Alloc1[T, P1] { def instantiate(p1: P1): T = construction.splice }
    }
  }
  ...
  def allocMacro2[T : c.WeakTypeTag, P1 : c.WeakTypeTag, P2 : c.WeakTypeTag](
    ...
    reify {
      new Alloc2[T, P1, P2] {
        def instantiate(p1: P1, p2: P2): T = construction.splice
      }
    }
  }

  def allocMacro3[
      T : c.WeakTypeTag, P1 : c.WeakTypeTag, P2 : c.WeakTypeTag, P3 : c.WeakTypeTag](
    ...
    reify {
      new Alloc3[T, P1, P2, P3] {
        def instantiate(p1: P1, p2: P2, p3: P3): T = construction.splice
      }
    }
  }

  def allocMacro4[
      T : c.WeakTypeTag, P1 : c.WeakTypeTag, P2 : c.WeakTypeTag, P3 : c.WeakTypeTag, P4 : c.WeakTypeTag](
    ...
    reify {
      new Alloc4[T, P1, P2, P3, P4] {
        def instantiate(p1: P1, p2: P2, p3: P3, p4: P4): T =
          construction.splice
      }
    }
  }
