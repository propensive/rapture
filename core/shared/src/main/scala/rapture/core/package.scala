...
object `package` {
  ...
  type CanBuildFrom[-From, -Elem, +To] = collection.generic.CanBuildFrom[
      From, Elem, To]
  ...
  implicit class EnrichedString(val string: String) extends AnyVal {
    def as[T](implicit parser: StringParser[T],
              mode: Mode[`String#as`]): mode.Wrap[T, parser.Throws] =
  ...
  implicit class EnrichedCollection[Coll[X] <: Seq[X]](val coll: Coll[String])
      extends AnyVal {
    def mapAs[T](implicit parser: StringParser[T],
                 cbf: CanBuildFrom[Coll[String], T, Coll[T]],
                 mode: Mode[`Seq#mapAs`]): mode.Wrap[Coll[T], parser.Throws] =
      mode.wrap[Coll[T], parser.Throws] {
        val b = cbf(coll)
        coll foreach { x =>
  ...
  def yCombinator[A, B](fn: (A => B) => (A => B)): A => B =
    fn(yCombinator(fn))(_)
  ...
  implicit class SeqExtras[A, C[A] <: Seq[A]](val xs: C[A]) {
    ...
    def intersperse[B >: A, That](between: B)(
    ...
    def zipWith[T](fn: A => T)(
        implicit bf: CanBuildFrom[C[A], (A, T), C[(A, T)]]): C[(A, T)] = {
      val b = bf(xs)
      xs.foreach { x =>
        b += ((x, fn(x)))
      }
  ...
  implicit class EnrichedCollectionCompanion[
      +C[X] <: collection.GenTraversable[X]](
...
private[core] object Strapped {
  ...
  implicit def toStrapped[T](opt: Option[T]): Strapped[T] =
    Strapped(opt.toList)
}
...
private[core] class Modal[G <: MethodConstraint, E <: Exception] {
  def apply[T](fn: => T)(
      implicit mode: Mode[G], typeTag: TypeTag[E]): mode.Wrap[T, E] =
    mode.wrap(fn)
}
