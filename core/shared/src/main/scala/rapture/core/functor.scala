...
trait Functor[+F[x] <: Functor[F, x], A] { functor =>
  ...
  def map[B](
      fn: A => B): F[B] { type Throws = functor.Throws with Exception } =
    emap[Exception](fn)
  ...
  class Emap[E <: Exception]() {
    def apply[B](fn: A => B)(implicit tt: ClassTag[E]
    ): F[B] { type Throws = functor.Throws with E } =
      functor.rawMap {
        case (a, m) =>
          try fn(a) catch {
            case e: Exception => m.exception[B, E](e.asInstanceOf[E])
          }
