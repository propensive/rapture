...
private[jawn] trait Extractors {
  ...
  implicit val jawnJNumExtractor: JsonCastExtractor[JNum] = JsonCastExtractor(
      JawnAst, DataTypes.Number)

  implicit val jawnStringExtractor: JsonCastExtractor[JString] =
    JsonCastExtractor(JawnAst, DataTypes.String)

  implicit val jawnAtomExtractor: JsonCastExtractor[JAtom] = JsonCastExtractor(
      JawnAst, DataTypes.Scalar)
