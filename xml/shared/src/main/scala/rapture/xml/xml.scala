...
private[xml] trait Xml_2 {
  implicit def xmlExtractorMacro[T <: Product, Th]: Extractor[T, Xml] {
    type Throws = Th
  } = macro XmlMacros.xmlExtractorMacro[T, Th]

  implicit def xmlSerializerMacro[T <: Product](
      implicit ast: XmlAst): Serializer[T, Xml] = macro XmlMacros
...
private[xml] trait XmlDataCompanion[
    +Type <: XmlDataType[Type, AstType], AstType <: XmlAst]
...
private[xml] trait XmlDataType[
    +T <: XmlDataType[T, AstType], AstType <: XmlAst]
...
object XmlBuffer extends XmlDataCompanion[XmlBuffer, XmlBufferAst] {
  ...
  def construct(any: MutableCell, path: Vector[Either[Int, String]])(
      implicit ast: XmlBufferAst): XmlBuffer = new XmlBuffer(any, path)
}
...
object Xml extends XmlDataCompanion[Xml, XmlAst] with Xml_1 {
  ...
  def construct(any: MutableCell, path: Vector[Either[Int, String]])(
      implicit ast: XmlAst): Xml = new Xml(any, path)

  def ast(xml: Xml): XmlAst = xml.$ast

  def extractor[T](implicit ext: Extractor[T, Xml]
      ): Extractor[T, Xml] { type Throws = ext.Throws } = ext
  def serializer[T](implicit ser: Serializer[T, Xml]) = ser

  implicit def xmlCastExtractor[T : XmlCastExtractor](
      implicit ast: XmlAst): Extractor[T, XmlDataType[_, _ <: XmlAst]] =
    new Extractor[T, XmlDataType[_, _ <: XmlAst]] {
      type Throws = DataGetException
      def extract(
          value: XmlDataType[_, _ <: XmlAst],
          ast2: DataAst,
          ...
          case ast2: XmlAst =>
            ...
            try {
              if (ast == ast2) norm.asInstanceOf[T]
                ...
                XmlDataType.xmlSerializer
                  .serialize(Xml.construct(MutableCell(norm), Vector())(ast2))
                  .asInstanceOf[T]
            } catch {
              case e: ClassCastException =>
                mode.exception[T, DataGetException](TypeMismatchException(
                        ast.getType(norm),
...
class Xml(
    val $root: MutableCell, val $path: Vector[Either[Int, String]] = Vector())(
    implicit val $ast: XmlAst)
    extends XmlDataType[Xml, XmlAst] with DynamicData[Xml, XmlAst] {
...
class XmlBuffer(
  ...
  def $deref(path: Vector[Either[Int, String]]): XmlBuffer =
    new XmlBuffer($root, path)
