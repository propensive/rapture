...
private[xml] trait Serializers {
  ...
  case class BasicXmlSerializer[T](serialization: T => Any)
      extends Serializer[T, Xml] {
    def serialize(t: T): Any = serialization(t)
  }

  implicit def xmlBufferSerializer[T](
      implicit ser: Serializer[T, Xml]): Serializer[T, XmlBuffer] =
    new Serializer[T, XmlBuffer] {
  ...
  implicit def booleanSerializer(
      implicit ast: XmlAst): Serializer[Boolean, Xml] =
    BasicXmlSerializer(ast fromString _.toString)

  implicit def stringSerializer(
      implicit ast: XmlAst): Serializer[String, Xml] =
  ...
  implicit def doubleSerializer(
      implicit ast: XmlAst): Serializer[Double, Xml] =
    BasicXmlSerializer(ast fromString _.toString)

  implicit def bigDecimalSerializer(
      implicit ast: XmlAst): Serializer[BigDecimal, Xml] =
    BasicXmlSerializer(ast fromString _.toString)

  implicit def bigIntSerializer(
      implicit ast: XmlAst): Serializer[BigInt, Xml] =
    BasicXmlSerializer(ast fromString _.toString)
  ...
  implicit def mapSerializer[Type, Ast <: XmlAst](
      implicit ast: Ast,
      ser: Serializer[Type, Xml]): Serializer[Map[String, Type], Xml] =
    new Serializer[Map[String, Type], Xml] {
      def serialize(m: Map[String, Type]) =
        ast.fromObject(m.mapValues(ser.serialize))
    }

  implicit def directXmlSerializer[T : DirectXmlSerializer](
      implicit ast: XmlAst): Serializer[T, Xml] =
    BasicXmlSerializer(
        obj =>
          xmlSerializer.serialize(Xml.construct(MutableCell(obj), Vector())(
                  ?[DirectXmlSerializer[T]].ast)))
