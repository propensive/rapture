...
private[xml] trait Serializers {
  ...
  case class BasicXmlSerializer[T](serialization: T => Any)
      extends Serializer[T, Xml] { def serialize(t: T): Any = serialization(t) }

  implicit def xmlBufferSerializer[T](implicit ser: Serializer[T, Xml]):
      Serializer[T, XmlBuffer] =
  ...
  implicit def traversableSerializer[Type, Coll[T] <: Traversable[T]]
      (implicit ast: XmlAst, ser: Serializer[Type, Xml]): Serializer[Coll[Type], Xml] =
    BasicXmlSerializer(ast fromArray _.map(ser.serialize).to[List])

  implicit def optionSerializer[Type]
      (implicit ast: XmlAst, ser: Serializer[Type, Xml]): Serializer[Option[Type], Xml] =
    BasicXmlSerializer(_ map ser.serialize getOrElse ast.nullValue)

  implicit def mapSerializer[Type, Ast <: XmlAst]
      (implicit ast: Ast, ser: Serializer[Type, Xml]): Serializer[Map[String, Type], Xml] =
    new Serializer[Map[String, Type], Xml] {
      def serialize(m: Map[String, Type]) = ast.fromObject(m.mapValues(ser.serialize))
    }

  implicit def directXmlSerializer[T: DirectXmlSerializer](implicit ast: XmlAst):
      Serializer[T, Xml] =
    BasicXmlSerializer(obj => xmlSerializer.serialize(Xml.construct(MutableCell(obj),
        Vector())(?[DirectXmlSerializer[T]].ast)))
