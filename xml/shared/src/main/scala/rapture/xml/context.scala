...
private[xml] object XmlDataMacros extends DataContextMacros[Xml, XmlAst] {
  ...
  def companion(c: BlackboxContext): c.Expr[DataCompanion[Xml, XmlAst]] =
    c.universe.reify(Xml)
    ...
    try {
      ...
      case XmlValidator.ValidationException(strNo, pos, expected, found) =>
        ...
        Some(
            (strNo,
             pos,
             s"failed to parse Xml literal: expected $expected, but found $f"))
      case XmlValidator.DuplicateKeyException(strNo, pos, key) =>
        Some((strNo, pos, s"""duplicate key found in Xml literal: "$key""""))
    }

  override def contextMacro(c: BlackboxContext)(
      exprs: c.Expr[ForcedConversion[Xml]]*)(
      parser: c.Expr[Parser[String, XmlAst]]): c.Expr[Xml] =
    super.contextMacro(c)(exprs: _*)(parser)
}

private[xml] object XmlBufferDataMacros
    extends DataContextMacros[XmlBuffer, XmlBufferAst] {

  def companion(
      c: BlackboxContext): c.Expr[DataCompanion[XmlBuffer, XmlBufferAst]] =
  ...
  override def contextMacro(c: BlackboxContext)(
      exprs: c.Expr[ForcedConversion[XmlBuffer]]*)(
...
private[xml] class XmlStrings(sc: StringContext) {
  class XmlContext() extends DataContext(Xml, sc) {
    def apply(exprs: ForcedConversion[Xml]*)(
        implicit parser: Parser[String, XmlAst]): Xml = macro XmlDataMacros.contextMacro
  }
...
private[xml] class XmlBufferStrings(sc: StringContext) {
  class XmlBufferContext() extends DataContext(XmlBuffer, sc) {
    def apply(exprs: ForcedConversion[XmlBuffer]*)(
        implicit parser: Parser[String, XmlBufferAst]): XmlBuffer = macro XmlBufferDataMacros.contextMacro
  }
