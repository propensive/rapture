...
private[xml] object XmlMacros {
  def xmlExtractorMacro[T : c.WeakTypeTag, Th](
      c: WhiteboxContext): c.Expr[Extractor[T, Xml] { type Throws = Th }] =
  ...
  def xmlSerializerMacro[T : c.WeakTypeTag](c: WhiteboxContext)(
      ast: c.Expr[XmlAst]): c.Expr[Serializer[T, Xml]] =
