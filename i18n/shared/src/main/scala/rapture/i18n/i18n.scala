...
object Locale {
  implicit def upcastLocale[From <: Language, ToLang <: From](
      loc: Locale[From]): Locale[ToLang] =
...
case class Locale[L <: Language : ClassTag]() {
  ...
  implicit val defaultLanguage: DefaultLanguage[L] =
    DefaultLanguage[L](classTag)
}

case class LocaleException(locale: String)
    extends Exception(s"Locale '$locale' not recognised.")

class LocaleParser[L <: Language](
    val locales: Map[String, Locale[_ >: L <: Language]]) {
  def |[L2 <: Language](locale: Locale[L2]) =
    new LocaleParser[L with L2](locales + (locale.name -> locale))

  def parse(s: String)(
      implicit mode: Mode[_]): mode.Wrap[Locale[L], LocaleException] =
    mode.wrap {
...
object I18n {
  ...
  implicit def upcast[ToLang <: Language, FromLang <: Language](
      from: I18n[String, FromLang]): I18n[String, ToLang] = macro Macros
    .missingTranslationsMacro[ToLang, FromLang]

  implicit def convertToType[
      T, L <: Language, L2 >: L <: Language : DefaultLanguage](
      i18n: I18n[T, L]): T =
    i18n.map(implicitly[DefaultLanguage[L2]].tag)

  class `I18n.apply`[L <: Language]() {
    def apply[T](value: T)(implicit ct: ClassTag[L]) =
      new I18n[T, L](Map(ct -> value))
  }
...
private[i18n] object RequireLanguage {
  implicit def requireLanguage: RequireLanguage[Language] = null
}
...
class I18n[T, Languages <: Language](private val map: Map[ClassTag[_], T]) {
  ...
  override def toString = {
    val langs = map.keys
      .map(_.runtimeClass.getName.takeRight(2).toLowerCase)
      .mkString("&")
    val content: Option[T] = map.get(implicitly[ClassTag[En]])
    lazy val first: Option[T] = map.headOption.flatMap {
      case (k, v) => map.get(k)
    }
...
object I18nStringParam {
  implicit def stringToStringParam[L <: Language](
      s: String): I18nStringParam[L] =
    new I18nStringParam[L](new I18n[String, L](Map()) {
      override def apply[Lang >: L](implicit ct: ClassTag[Lang]) = s
    })

  implicit def toI18nStringParam[L <: Language](
      s: I18n[String, L]): I18nStringParam[L] =
    I18nStringParam(s)
}
case class I18nStringParam[+L <: Language](
    i18n: I18n[String, L @annotation.unchecked.uncheckedVariance])
    extends AnyVal
