...
private[css] object CssMacros {
  ...
  def stylesheetContextMacro(c: BlackboxContext)(exprs: c.Expr[ForcedConversion[CssStylesheet]]*): c.Expr[CssStylesheet] = {
    import c.universe._
...
private[css] class CssStrings(sc: StringContext) {
  class CssContext() {
    def apply(exprs: ForcedConversion[Css]*): Css =
      macro CssMacros.contextMacro
  }
  class StylesheetContext() {
    def apply(exprs: ForcedConversion[CssStylesheet]*): CssStylesheet =
      macro CssMacros.stylesheetContextMacro
  }
