...
private[css] object CssMacros {
  ...
  def parseSource(s: List[String],
                  stringsUsed: List[Boolean],
                  stylesheet: Boolean): Option[(Int, Int, String)] =
    try {
  ...
  def contextMacro(c: BlackboxContext)(
      exprs: c.Expr[ForcedConversion[Css]]*): c.Expr[Css] = {
    import c.universe._
    import compatibility._
